...
bool ESPWebFileManager::deleteRecursive(fs::FS& fs, const String& path) {
	// Protect "config.ini" in the root directory
	String normalized = path;
	normalized.toLowerCase();
	if (normalized == "/config.ini") {
		return false;
	}
	...
	if (file.isDirectory()) {
		...
		file.close();
		return fs.rmdir(path); // delete now-empty folder
	}
	else {
		file.close();
		return fs.remove(path); // if it's just a file, remove it
	}
}
